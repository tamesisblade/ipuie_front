...
<vx-card :title="'Tareas de la sección: ' + seccion">
  <vs-table search max-items="10" pagination :data="tareas">
                  ...
                  <div class="flex">
                      ...
                      <vx-tooltip color="danger" text="Eliminar tarea">
                       ...
                       <vs-td :data="tr.idInstitucion">
                            <div class="flex">
                                <vs-button  color="primary" @click="calificar(tr)">Ver tareas</vs-button>
                            </div>
                        </vs-td>
                  </div>
              </vs-td>
    ...
    <!--Modal para ver tareas de los estudiantes -->
        <vs-popup class="popupActividad" fullscreen  title="Tareas de los estudiantes" :active.sync="popupTareaEstudiantes">
            <div>
                    ...
                    <template slot-scope="{data}">
                        <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                            <vs-td :data="data[indextr].nombre">
                                 ...
                                 <a :href="$server_url_file+'tareas/'+data[indextr].url" target="_blank">
                            ...
                            <vs-td>
                                <div style="display:flex;">
                                    <vs-button type="border" size="small" icon-pack="feather" icon="icon-edit" class="mr-2" @click="changeCalificacion(tr)" color="success">

                                    </vs-button>
                                    <vs-button color="danger" size="small" type="gradient" @click="confirmEliminarCalificacion(tr)" >
                                        X TAREA
                                    </vs-button>   
                                </div>    
        ...
        <vs-prompt
            ...
            <div class="con-exemple-prompt">
                ...
                <vs-input type ="number"  class="inputx mb-3 w-full" label="Ingrese una calificacion" max="3" v-model="calificacion.nota" />   
                <vs-textarea class="w-full mb-4 p-2" label="Descripción" v-model="calificacion.comentario" />
...
export default {
    ...
    methods: {
      calificar(item){
            ...
            me.$vs.loading()
            let docente= "yes";
            me.$http.get(me.$server_url+'tareaEstudianteRealizada?id_seccion=' +item.id_seccion + '&docente='+docente+'&id_tarea='+item.id_tarea)
        ...
        eliminarTarea(id_tarea){
          let me = this
          axios.get("http://localhost:8000/api/eliminar_tarea?id_tarea="+id_tarea)
          .then(function (response) {
            me.$vs.loading.close()
        ...
        editCalificacion(){
            ...
            me.$vs.loading({
               ...
               axios.post("http://localhost:8000/api/editarCalificacionTarea", formData)
        ...
        confirmEliminarCalificacion(item){
           ...
           ElimininarActividadEstudiante(){
            ...
            me.$http.post(this.$server_url+'eliminarTareaEstudiante', formData)
