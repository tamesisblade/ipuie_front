                  ...
                  <vs-td>
                      <div class="flex">
                            ...
                            <vx-tooltip text="eliminar sección">
                              <vs-button radius type="line" size="large" icon-pack="feather" icon="icon-trash" class="m-1" color="danger" @click="id_carrusel=tr.id_carrusel; img_carrusel=tr.imagen; openConfirmEliminarCarrusel()"></vs-button>
                            </vx-tooltip>
                      </div>
                  </vs-td>
              </vs-tr>
...
export default {
    ...
    methods: {
      ...
      elimiarLadoCubo(id_cubo, img){
        let me = this

        me.$vs.loading()
        axios.get('http://localhost:8000/api/eliminar_img_cubo/' + id_cubo+'/'+img)
        .then(function (response) {
            me.$vs.loading.close()
            me.$vs.notify({
      ...
      openConfirmEliminarCarrusel(){
        this.$vs.dialog({
          type: 'confirm',
          color: 'danger',
          title: `Confirmar`,
          text: '¿Está seguro de eliminar esta sección?',
          accept: this.eliminarCarrusel
        })
      },
      eliminarCarrusel(){
          let me = this
          axios.get("http://localhost:8000/api/eliminar_carrusel/"+me.id_carrusel+'/'+me.img_carrusel)
          .then(function (response) {
              me.getCarruseles()
          })
