...
export default {
    ...
    methods: {
        getTareas(id) {
            ...
            var url = this.$server_url+'tareaEstudiantePendiente?id_seccion=' + me.id_seccion + "&idusuario=" + me.usuario[0].idusuario;
            axios.get(url)
        ...
        realizar(item){
            localStorage.idtarea = item.id_tarea;
            localStorage.realizar = "tarea";
            this.$router.push('/estudiante/tarea/add');
        }
    },
}
</script>
